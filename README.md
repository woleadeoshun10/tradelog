...
- **Orders / Setups mode** — toggle between individual orders and grouped setups (trades within 10 minutes, same pair/direction/account count as one setup)
- **Trade log** — full table of all trades with date, pair, direction, session, entry/exit, P&L, R:R, source, edit, and quick delete
- **Edit trades** — edit any trade directly from the detail modal
...
- **Trade journals** — per-trade journal entries with setup notes, execution, lessons, confidence rating, emotional state, R:R, strategy tag, mistake tags, and screenshot uploads
- **Timeframe filters** — 1D / 1W / 1M / 3M / 6M / 1Y / ALL + custom date range
- **Streak** — based on daily P&L, not individual trades
- **Timezone selector** — display all trade times in your local timezone
...
| Auth & DB | [Supabase](https://supabase.com) (email auth + PostgreSQL + Storage) |
...
Toggle between **Orders** (individual positions) and **Setups** (grouped trades) in the Trade Log header.

Grouping rules:
- Same pair, direction, and account
- Opened within **10 minutes** of each other
- Same trading day

When grouped, the setup shows:
- **P&L** = sum of all positions
- **Lots** = sum of all lots
- **Entry / Exit** = weighted average by lot size
- **Result** = Win / Loss / BE based on total P&L
- A gold badge showing how many orders are inside (e.g. `3x`)

Win rate, streaks, dashboard, and weekly stats all reflect setups when Setups mode is active.
...
supabase-setup.sql  — database and storage setup script
README.md           — this file
```
